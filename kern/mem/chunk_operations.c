...
//	return;
	...
	uint32 sizeToMark=virtual_address+size;
	//struct FrameInfo * allocptr=NULL;
	uint32 *Page_Table_Ptr = NULL;
	//cprintf("in alloc user mem before loop \n");
	for (uint32 i=virtual_address;i<sizeToMark;i+=PAGE_SIZE)
	{
		int ret;
		ret=get_page_table(e->env_page_directory,i,&Page_Table_Ptr);
		if(ret==TABLE_NOT_EXIST)
		{
			create_page_table(e->env_page_directory,i);
			//cprintf("in alloc user mem in con in loop \n");

		}
		//cprintf("in alloc user mem after con in loop \n");
		pt_set_page_permissions(e->env_page_directory,i,PERM_MARKED,0);
		//allocate_frame(&allocptr);
		//map_frame(e->env_page_directory,allocptr,virtual_address,PERM_PRESENT|PERM_WRITEABLE);
	}
...
void free_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	if(e->page_last_WS_element!=NULL&&e->page_last_WS_element!=e->page_WS_list.lh_first)
	{
		//tail next hyroo7 ll head
		e->page_WS_list.lh_last->prev_next_info.le_next=e->page_WS_list.lh_first;
		//head prev hyroo7 ll tail
		e->page_WS_list.lh_first->prev_next_info.le_prev=e->page_WS_list.lh_last;
		//fifo prev next b null
		e->page_last_WS_element->prev_next_info.le_prev->prev_next_info.le_next=NULL;

		//tail hyroo7 ll prev fifo
		e->page_WS_list.lh_last=e->page_last_WS_element->prev_next_info.le_prev;
		//prev fifo b null
