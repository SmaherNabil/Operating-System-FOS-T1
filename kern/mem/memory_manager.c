...
int allocate_frame(struct FrameInfo **ptr_frame_info)
{
	*ptr_frame_info = LIST_FIRST(&free_frame_list);
	...
	if (*ptr_frame_info == NULL)
	{
		...
		int removedExit = 0;
		//struct Env * tmpE = NULL;
		if(env_exit_queue.size != 0){
			struct Env * t = env_exit_queue.lh_first;
			for(int i=0; i<LIST_SIZE(&(env_exit_queue)); i++){
				env_free(t);
				removedExit = 1;
				t = t->prev_next_info.le_next;
			}
		}
		else {
			for(int i=0; i<num_of_ready_queues; i++){
				struct Env* tmpE=env_ready_queues[i].lh_first;
				for (int j=0;j<env_ready_queues[i].size;j++){
					if(tmpE->env_status == ENV_EXIT){
						env_free(tmpE);
						removedExit = 1;
					}
					if(tmpE->prev_next_info.le_next!=NULL)
					{
						tmpE=tmpE->prev_next_info.le_next;}
					else
						break;
				}
			}
		...
		if(removedExit==0){
			...
			if (isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX)){
				//remove from each process
				for(int i=0; i<num_of_ready_queues; i++){
					struct Env* tmpE=env_ready_queues[i].lh_first;

					for (int j=0;j<env_ready_queues[i].size;j++){

					//if second list not empty remove last one from second list
					if(tmpE->SecondList.size != 0){
						...
						env_page_ws_invalidate(tmpE, victim);
					}
					//else remove last one from active list
					else if(tmpE->ActiveList.size != 0){
						uint32 victim = curenv->ActiveList.lh_last->virtual_address;
						int found = pt_get_page_permissions(curenv->env_page_directory,
								victim);
						if ((found & PERM_MODIFIED) == PERM_MODIFIED) {
				...
				//iterate over all processes / environments
				for(int i=0; i<num_of_ready_queues; i++){
					struct Env* tmpE=env_ready_queues[i].lh_first;

					for (int j=0;j<env_ready_queues[i].size;j++){

					if(tmpE->page_WS_list.size != 0){
						uint32 victim = tmpE->page_last_WS_element->virtual_address;
						int found = pt_get_page_permissions(curenv->env_page_directory,
								victim);
						if ((found & PERM_MODIFIED) == PERM_MODIFIED) {
