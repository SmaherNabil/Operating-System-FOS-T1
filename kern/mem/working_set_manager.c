...
inline struct WorkingSetElement* env_page_ws_list_create_element(struct Env* e, uint32 virtual_address)
{
	...
	//create an object
	struct WorkingSetElement * elemOfWS;
	//size of the element
	int sizeOfElement=sizeof(struct WorkingSetElement);
	//allocate in kernel heap
	elemOfWS=(struct WorkingSetElement * )kmalloc(sizeOfElement);
	//if kmalloc could not allocate page for WS element
	if (elemOfWS == NULL) {
		panic("kmalloc could not allocate page for WS element. loser \n");
	}
	elemOfWS->virtual_address=virtual_address;
	elemOfWS->empty=1;
	return elemOfWS;
