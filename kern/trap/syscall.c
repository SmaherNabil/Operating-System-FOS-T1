...
void* sys_sbrk(int increment)
{
	...
	int numOfPages = (localSegBrk - oldSegBrk) / PAGE_SIZE;
		...
		if (increment == 0) {
			...
			if (localSegBrk > env->UH_limit) {
				return (void*)-1;
			}
			else{
			env->UH_break=localSegBrk;
			allocate_user_mem(env,oldSegBrk,localSegBrk-oldSegBrk);
			return (void*) oldSegBrk;
			}
		}
		else if (increment < 0) {
			//if the increment is lower than the start
			if (env->UH_break + increment < env->UH_start) {
				return (void*)-1;
			}
//			else if (-(increment) < PAGE_SIZE)
//			{
//				env->UH_break += increment;
//				return (void*) env->UH_break;
//			}
			else
			{
				for(uint32 va =localSegBrk; va < env->UH_break; va += PAGE_SIZE) {
					free_user_mem(env,localSegBrk,oldSegBrk-localSegBrk);
				    }
//				int pagesToFree = -increment / PAGE_SIZE;
//				for (int i = 0; i < pagesToFree; i++)
//				{
//					free_user_mem(env,oldSegBrk,localSegBrk-oldSegBrk);
//					env->UH_break -= PAGE_SIZE;
//				}
//				int extra = -increment % PAGE_SIZE;
				env->UH_break =oldSegBrk+increment;
				return (void *) env->UH_break;
			}
		}
		//MS2: COMMENT THIS LINE BEFORE START CODING====
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_free_user_mem:
			if ((a1 >=(USER_LIMIT-PAGE_SIZE)&&a2>=PAGE_SIZE+10)||a1==0||a1>= USER_LIMIT)
				{
	...
	case SYS_env_set_nice:
			env_set_nice((struct Env*)a1,(int)a2);
			return 0;
			break;
	//=====================================================================
	case SYS_cputs:
