...
void env_set_nice(struct Env* e, int nice_value)
{
  ...
  if(e->env_status!=ENV_NEW)
	{
		fixed_point_t b4Round = fix_sub(fix_int(PRI_MAX),
				fix_sub(fix_unscale(e->recent,4),fix_int(nice)));
		int rounded = fix_round(b4Round);

		//in range PRI_min & PRI_max
		if(rounded < PRI_MIN){
		rounded = PRI_MIN;
		}

		if(rounded>num_of_ready_queues-1){
		rounded = num_of_ready_queues-1;
		}
		e->priority = rounded;
	}
...
int env_get_recent_cpu(struct Env* e)
{
	...
	int recent=fix_round(fix_scale(e->recent,100));
	return recent;
}
int get_load_average()
{
	//TODO: [PROJECT'23.MS3 - #3] [2] BSD SCHEDULER - get_load_average
	int loadAvg=fix_round(fix_scale(load_avg,100));
	return loadAvg;
}
/********* for BSD Priority Scheduler *************/
//==================================================================================//
