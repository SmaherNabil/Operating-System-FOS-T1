...
int process_command(int number_of_arguments, char** arguments)
{
	...
	//if all correct - return ind - empty list
	//if correct but X num_of_arg - return CMD_INV_NUM_ARGS - found cmd
	//if not correct but matches some - return CMD_MATCHED - found cmd(s)
	//if incorrect - return CMD_INVALID - empty list

	int found = 0;
	int cmd_valid=0;
	int i = 0;
	//to reset foundCommands
	LIST_INIT(&foundCommands);

	//for each word command in list commands
	for (i=0; i < NUM_OF_COMMANDS; i++){
		//resetting letters matched between each command
		int nletters_matched=0;
		bool first=0;
		int x = strcmp(arguments[0], commands[i].name);

		if(x==0){
			//REMOVED -1!!! ADD IT BACK LATER
			int arg_num=number_of_arguments-1;
			if(commands[i].num_of_args==-1 && arg_num>=1){
				return i;
			}
			if(arg_num == commands[i].num_of_args){
				return i;
			}
			else {
				//one time panic one time correct
				//reset in case any command added in matched char
				LIST_INIT(&foundCommands);
        ...
        char * indCurrent = &commands[i].name[0];
        //bool first = 0;

			//for each letter in input command
			for (int letter = 0; letter < strlen(arguments[0]) ; letter++){

				//pointer at index of found char
				char * indFound = strfind(commands[i].name, arguments[0][letter]);

				//if letter found in command[i]
				//general case
				if((*indFound)==arguments[0][letter]&&indFound>indCurrent){
					nletters_matched++;
					indCurrent=indFound;
				}
				//in case 1st char in command matches
				else if(indFound == &commands[i].name[0]&&indFound == indCurrent && first==0){
				   first = 1;
				   if((*indFound)==arguments[0][letter]){
				   	  nletters_matched++;
				   	  //indCurrent=indFound;
				  }
				}
				//so it doesn't continue the rest of the letters as soon as 1 isn't found
				else break;
			}
			if(nletters_matched == strlen(arguments[0])){
				LIST_INSERT_HEAD(&foundCommands, &commands[i]);
				cmd_valid++;
			}
		  }
